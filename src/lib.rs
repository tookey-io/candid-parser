...
fn parse(input: String) -> Result<BTreeMap<String, TypeInner>> {
  let mut env = TypeEnv::new();
  let prog: IDLProg = input.parse().map_err(|e| anyhow!("{}", e))?;
  let prog = check_prog(&mut env, &prog).map_err(|e| anyhow!("{}", e))?;
  let mut types: BTreeMap<String, TypeInner> = BTreeMap::new();

  env.0.iter().for_each(|(key, value)| {
    types.insert(key.to_owned(), TypeInner::from(value));
  });

  if let Some(prog) = prog {
    types.insert("Service".to_owned(), TypeInner::from(&prog));
  }

  Ok(types)
}
#[napi]
pub fn parse_idl_pretty(input: String) -> Result<String> {
  Ok(serde_json::to_string_pretty(&parse(input)?)?)
}

#[napi]
pub fn parse_idl(input: String) -> Result<String> {
  Ok(serde_json::to_string(&parse(input)?)?)
}
